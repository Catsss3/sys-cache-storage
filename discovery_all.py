...
def fetch(url, params=None, headers=None, method="GET", json_data=None):
    hdr = {"User-Agent": USER_AGENT, "Accept-Language": "en-US,en;q=0.9"}
...
def gemini_search():
    ...
    for q in queries:
        payload = {"contents": [{"parts": [{"text": f"Search the web for {q} and return ONLY a plain list of URLs."}]}]}
...
def github_code_search():
    ...
    params = {"q": "sub in:url extension:txt extension:json", "per_page": 30}
...
def tg_web_crawl():
    aggregators = ["proxylistdaily", "v2raycollector", "v2ray_free_conf", "v2rayng_org"]
    new_ch, new_sub = set(), set()
...
def ddg_search():
    queries = ['site:t.me "hysteria2"', 'site:github.com "proxy list" sub']
    new_ch, new_sub = set(), set()
    for q in queries:
        resp = fetch("https://duckduckgo.com/html/", params={"q": q})
        if resp:
            c, s = extract_links(resp.text)
            new_ch.update(c); new_sub.update(s)
    return new_ch, new_sub
...
def main():
    ...
    ddg_ch, ddg_sub = ddg_search()
    
    # Объединяем
    all_found = existing | g_ch | g_sub | gists | gh_code | tw_ch | tw_sub | ddg_ch | ddg_sub
    
    # Фильтруем (убираем пустые и мусор)
    all_found = {link.strip() for link in all_found if len(link.strip()) > 10}
